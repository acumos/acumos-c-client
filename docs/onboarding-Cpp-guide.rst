...
1. Train the model
2. Serialize trained model
3. Create Microservice
4. Define the gRPC service
5. Use the onboarding-cpp client to save model_bundle locally or to onboard it to Acumos by CLI 
...
and finally train the model and save it in serialized format
...
and launch the cpp client by command line if you want or with your preferred Python IDE. It is recommended to call the onboarding
 script from /examples/iris-kmeans folder.

.. code:: bash
    python3 ../../cpp-client.py

The Acumos on-boarding cpp client will ask you the follwing question (use Tab-key for completion) :
...
Please refer to tutorial to use the onboarding-cpp client.
